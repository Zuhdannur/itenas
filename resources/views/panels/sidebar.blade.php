            ...
            <li class="nav-item mr-auto"><a class="navbar-brand" href="{{ url('/') }}"><span class="brand-logo">
                        <div class="d-flex justify-content-center align-middle">
                            <img src="{{ $appIcon == null ? asset('images/portrait/small/avatar-s-11.jpg') : asset('storage/' . $appIcon->filename) }}"
                                class="rounded mr-50" alt="profile image" height="30" width="30" />
                            <h2 class="brand-text" style="margin-top: 2px">Itenas</h2>
                        </div>
                </a></li>
            ...
            @if (isset($menuData[0]))
                @foreach ($menuData[0]->menu as $menu)
                    ...
                    @else
                        ...
                        @endphp
                        @if (Auth::user()->role->name == 'admin' || empty($menu->role))
                            <li
                                class="nav-item {{ strpos(Route::currentRouteName(), $menu->slug) !== false ? 'active' : '' }} {{ $custom_classes }}">
